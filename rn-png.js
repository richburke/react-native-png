...
export default class RnPng {
  ...
  getPixelAt(pos) {
     ...
     * @todo
     */
    // If we have a PLTE, handle that
    // - get color data based upon pixel
    // - If there's a tRNS, get the data for that
    return pixel;
  }
  ...
  setPixelAt(pos, data) {
    ...
    const fullPixelSize = determineFullPixelSize(_colorType.get(this));
    ...
    if (this.isIndexed()) {
      ...
      if ('undefined' !== typeof opacityData) {
        this.setTransparency(colorData, paletteIndex);
      }
